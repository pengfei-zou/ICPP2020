...
#PBS -o /scratch3/pzou/powerv100.mybench.o
#PBS -e /scratch3/pzou/powerv100.mybench.e
...
mkdir ${scratch_results}/mybench/v100/power-${nodeName}-${current_date_time}
mkdir ${home_results}/mybench/v100/power-${nodeName}-${current_date_time}


python ./power.py ./applications_mybench_re.csv &
pid=$!
...
while ps -p $pid >/dev/null 2>&1; do
	sleep 600
    rm $TMPDIR/CUPTI*
	rsync -az $TMPDIR/ ${scratch_results}/mybench/v100/power-${nodeName}-${current_date_time}
    rsync -az $TMPDIR/ ${home_results}/mybench/v100/power-${nodeName}-${current_date_time}
